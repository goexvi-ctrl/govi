...
var exCmdMeta = map[string]exCmdUsage{
	...
	"*":            {"execute a buffer", ":* [buffer]"},
	"#":            {"display numbered lines", ":[range] # [count]"},
	...
	"bg":           {"background the current screen", ":bg"},
	"fg":           {"foreground a background screen", ":fg [file]"},
	"resize":       {"grow or shrink the current screen", ":res[ize] [+-]rows"},
	"display":      {"display buffers, connections, screens, or tags", ":display b[uffers] | c[onnections] | s[creens] | t[ags]"},
	"cscope":       {"create a set of tags using a cscope command", ":cs[cope] command [args]"},
	...
	"tagnext":      {"move to the next tag", ":tagn[ext]"},
	"tagpop":       {"pop one tag off the tags stack", ":tagp[op]"},
	"tagprev":      {"move to the previous tag", ":tagp[rev]"},
	"tagtop":       {"pop all tags off the tags stack", ":tagt[op]"},
	"t":            {"copy lines", ":[range] t address"},
	"unabbreviate": {"remove an abbreviation", ":una[bbreviate] lhs"},
	"undo":         {"undo the most recent change", ":u[ndo]"},
